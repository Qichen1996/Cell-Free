...
class UserEquipment:
    ...
    #     res = False                                          
    #     for i in self._sorted_bss[:self.cluster_size]:        # **只看前 C 个**
    #         bs = self.net.get_bs(i)
    #         if bs in self.bss:                                # 已连接就跳过
    #             continue
    #         if bs.respond_connection_request(self):
    #             bs.add_to_cell(self)
    #             res = True
    #             if len(self.bss) >= self.cluster_size:        # 已够用，停止
    #                 break
    #     return res
